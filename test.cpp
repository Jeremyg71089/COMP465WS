...
// Model for shapes
char * modelFile = "sphere-r50.tri"; // name of tri model file
...
void display(void) {
  ...
  if ( timeInterval >= 1000) {
    sprintf(fpsStr, " distance %4d", (int) (eye.z) );
...
void keyboard (unsigned char key, int x, int y) {
  switch(key) {
    ...
    case 'f' : case 'F' :  // front view
		eye.z+= 100;
        //eye = glm::vec3(0.0f, 0.0f, 20000.0f);   // eye is 2000 "out of screen" from origin
        at  = glm::vec3(0.0f, 0.0f,    0.0f);   // looking at origin
        up  = glm::vec3(0.0f, 1.0f,    0.0f);   // camera'a up vector
...
# endif
  ...
  std::cout << "MODEL MATRIX: "<< glm::to_string(modelMatrix);
