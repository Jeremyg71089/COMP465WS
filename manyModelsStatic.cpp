...
glm::mat4 projectionMatrix;     // set in reshape()
glm::mat4 ModelViewProjectionMatrix; // set in display();
glm::vec3 unumPos = translate[2], duoPos = translate[3];
...
void display() {
	...
	for (int m = 0; m < nModels; m++) {
		...
		//the regular equation for rotating around the y axis
		else {
			modelMatrix = rotation[m] * glm::translate(glm::mat4(), translate[m]) * glm::scale(glm::mat4(), glm::vec3(scale[m]));
		}
...
// Animate scene objects by updating their transformation matrices timerDelay = 40, or 25 updates / second
void update(int i) {
...
// Estimate FPS, use for fixed interval timer driven animation
void intervalTimer(int i) {
	glutTimerFunc(timerDelay, intervalTimer, 8);
	if (!idleTimerFlag) update();  // fixed interval timer
}
